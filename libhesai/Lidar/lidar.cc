...
int Lidar<T_Point>::Init(const DriverParam& param) {
  ...
  if (param.input_param.source_type == DATA_FROM_LIDAR && param.input_param.use_someip) {
    ...
    source_tmp->Open();
    auto ret = source_tmp->sendsomeip_subscribe_ipv4(param.input_param.device_ip_address,
                                                     param.input_param.host_ip_address,
                                                     param.input_param.udp_port,
                                                     param.input_param.fault_message_port);
    if (ret == false) {
  ...
  if (param.input_param.source_type == DATA_FROM_PCAP) {
    ...
    source_ = std::make_shared<SocketSource>(param.input_param.udp_port,
                                             param.input_param.multicast_ip_address);
    source_->Open();
    if (param.input_param.fault_message_port > 0 &&
        param.input_param.fault_message_port != param.input_param.udp_port) {
      source_fault_message_ = std::make_shared<SocketSource>(
          param.input_param.fault_message_port, param.input_param.multicast_ip_address);
      source_fault_message_->Open();
    }
  } else if (param.input_param.source_type == DATA_FROM_SERIAL) {
    source_ = std::make_shared<SerialSource>(param.input_param.rs485_com,
                                             param.input_param.rs485_baudrate,
                                             param.input_param.point_cloud_baudrate);
    source_rs232_ = std::make_shared<SerialSource>(param.input_param.rs232_com,
                                                   param.input_param.rs232_baudrate);
  ...
  while (GetGeneralParser() == nullptr && init_running) {
    ...
    if (param.input_param.recv_point_cloud_timeout >= 0 &&
        elapsed_seconds.count() > param.input_param.recv_point_cloud_timeout)
      break;
    if (ret == -1) {
  ...
  if (GetGeneralParser() == nullptr) {
    ...
    if (param.input_param.source_type == DATA_FROM_LIDAR &&
        param.input_param.device_ip_address == "" && param.input_param.use_ptc_connected) {
...
template <typename T_Point>
int Lidar<T_Point>::ParserFaultMessage(UdpPacket& udp_packet,
...
int Lidar<T_Point>::LoadCorrectionForUdpParser() {
  ...
  if (udp_parser_) {
    return udp_parser_->LoadCorrectionString((char*)sData.data(), sData.size());
  } else {
    LogError("udp_parser_ nullptr");
    return -1;
  }
...
int Lidar<T_Point>::LoadFiretimesForUdpParser() {
  ...
  if (udp_parser_) {
    return udp_parser_->LoadFiretimesString((char*)sData.data(), sData.size());
  } else {
    LogError("udp_parser_ nullptr");
    return -1;
  }
      ...
      default:
        if (len > 0) {
          ...
          if (!(udp_packet.buffer[0] == 0xCD && udp_packet.buffer[1] == 0xDC) &&
              !(udp_packet.buffer[SOMEIP_OFFSET] == 0xCD &&
                udp_packet.buffer[SOMEIP_OFFSET + 1] == 0xDC))
            is_timeout_ = false;
        }
        break;
    }
...
void Lidar<T_Point>::ReceiveUdpThreadFaultMessage() {
  ...
  while (running_) {
    ...
    if (len != kFaultMessageLength &&
        !(udp_packet_fault_message.buffer[0] == 0xCD &&
          udp_packet_fault_message.buffer[1] == 0xDC) &&
        !(udp_packet_fault_message.buffer[SOMEIP_OFFSET] == 0xCD &&
          udp_packet_fault_message.buffer[SOMEIP_OFFSET + 1] == 0xDC)) {
      continue;
    }
    ...
    if (is_record_pcap_) {
      pcap_saver_->Dump(udp_packet_fault_message.buffer,
                        udp_packet_fault_message.packet_len,
                        fault_message_port_);
    }
  }
  return;
...
void Lidar<T_Point>::SetThreadNum(int nThreadNum) {
  if (nThreadNum > GetAvailableCPUNum() - 2) {
    ...
    LogWarning("The number of threads is too large, set to the maximum number of threads %d.",
               nThreadNum);
  }
  ...
  if (nThreadNum > 1) {
    ...
    for (int i = 0; i < nThreadNum; i++) {
      handle_thread_vec_[i] =
          new std::thread(std::bind(&Lidar::HandleThread, this, std::placeholders::_1), i);
    }
  }
  ...
  parser_thread_ptr_ = std::make_shared<std::thread>(std::bind(&Lidar::ParserThread, this));
}
