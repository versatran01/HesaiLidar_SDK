...
// udp packet data will be received in udp data Receive thread and put into origin_packets_buffer_
// you can get origin udp packet data from origin_packets_buffer_ when you need
// you can put decoded packet into decoded_packets_buffer_ , parser thread will convert it
// pointsxyzi info in variable frame_ you can control lidar with variable ptc_client_
...
class Lidar {
 public:
  ...
  // set lidar type for Lidar object and udp parser, udp parser will initialize the corresponding
  // subclass
  int SetLidarType(std::string lidar_type);
  // get one packet from origin_packets_buffer_,
  int GetOnePacket(UdpPacket& packet);
  // start to record pcap file with name record_path,the record source is udp data Receive from
  // source
  ...
  // start to record pcap file with name record_path,the record source is the parameter
  // UdpFrameArray_t port is the udp port recorded in pcap
  ...
  // covert a origin udp packet to decoded data, and pass the decoded data to a frame struct to
  // reduce memory copy
 ...
 private:
  bool init_finish_[TotalStatus];  // 0: 基本初始化完成， 1：ptc初始化完成，
                                   // 2：角度校准文件加载完成， 3：全部初始化完成
  ...
  // Receive udp data thread, this function will Receive udp data in while(),exit when variable
  // running_ is false
  void ReceiveUdpThread();
  void ReceiveUdpThreadFaultMessage();
  /* parser decoded packet data thread. when thread num <  2, this function will parser decoded
  packet data in this thread when thread num >= 2,decoded packet data will be put in
  handle_thread_packet_buffer_, and the decoded packet will be parered in handle_thread*/
  void ParserThread();
  /* this function will parser decoded packet data  in handle_thread_packet_buffer_ into pointxyzi
   * info  */
